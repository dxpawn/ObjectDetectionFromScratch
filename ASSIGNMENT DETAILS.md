...
**Không được sử dụng các bộ phát hiện đối tượng hoàn chỉnh như YOLOv5/v8, Detectron2, MMDetection, hoặc Faster R-CNN/SSD có sẵn trong torchvision**. 
Được phép dùng PyTorch, các lớp mạng cơ bản, và mạng trích xuất đặc trưng đã huấn luyện trước (nếu các mô hình này được huấn luyện từ imagenet).
...
Nhân viên có thể chọn một trong các hướng:
...
# Chấm Tự Động
Nhân viên có thể tự kiểm tra định dạng và điểm trên tập huấn luyện/kiểm định bằng tệp chấm đi kèm trong thư mục `public/`. Ví dụ, sau khi xuất dự đoán cho tập kiểm định:
```
...
Thang điểm kết quả (thang chấm thực tế của môn học):
...
Lưu ý: công cụ đi kèm `public/tools/evaluate_predictions.py` dùng thang nội bộ
dễ hơn (`>= 0.50` đã là 20 điểm). Thang chấm thực tế ở trên mới là thang nghiêm
ngặt được dùng để chấm điểm cuối cùng.
